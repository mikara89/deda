...
for code in 401 403 429 500; do before=$(latest_request_at); set_state "{\"mode\":{\"status\":$code}}"; wait_for_all_provider_observations_after "$before" "all provider observations for HTTP $code"; [[ $(replicas github-runner) == 3 && $(replicas azure-runner) == 3 && $(replicas gitlab-runner) == 3 ]] || fail_scenario "failsafe hold did not preserve capacity for HTTP $code"; done
before=$(latest_request_at); set_state '{"mode":{"status":200,"body":"not-a-provider-document"}}'; wait_for_all_provider_observations_after "$before" 'all provider observations for semantically malformed records'
...
before=$(latest_request_at); set_state '{"mode":{"status":200,"body":null,"rawBody":"{"}}'; wait_for_all_provider_observations_after "$before" 'all provider observations for malformed JSON'
...
before=$(latest_request_at); set_state '{"mode":{"status":200,"body":null,"rawBody":null,"delaySeconds":20}}'; wait_for_all_provider_observations_after "$before" 'all provider observations for timeout'
...
before=$(latest_request_at); wait_for_all_provider_observations_after "$before" 'all provider recovery observations'
