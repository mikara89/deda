...
set_state '{"gitlab":{"jobs":[]}}'; wait_replicas gitlab-runner 0
docker service update --label-add com.deda.autoscale.trigger.runUntagged=true "$(service gitlab-runner)" >/dev/null
set_state '{"gitlab":{"jobs":[{"status":"pending","tag_list":[]}]}}'; wait_replicas gitlab-runner 1
set_state '{"gitlab":{"jobs":[]}}'; wait_replicas gitlab-runner 0
docker service update --label-add com.deda.autoscale.trigger.runUntagged=false "$(service gitlab-runner)" >/dev/null
