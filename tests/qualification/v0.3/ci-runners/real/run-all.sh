...
[[ -f "$root/result.json" ]] && deterministic=$(jq -r .fullDeterministicQualification "$root/result.json")
candidate_matched=1
...
if [[ ! -f "$manifest" ]]; then
  ...
  deterministic_github_image=$(jq -r .githubRunnerImage "$manifest")
  deterministic_github_digest=$(jq -r .githubRunnerReferenceDigest "$manifest")
  deterministic_azure_image=$(jq -r .azureRunnerImage "$manifest")
  deterministic_azure_digest=$(jq -r .azureRunnerReferenceDigest "$manifest")
  deterministic_gitlab_image=$(jq -r .gitlabRunnerImage "$manifest")
  deterministic_gitlab_digest=$(jq -r .gitlabRunnerReferenceDigest "$manifest")
  [[ -n "$deterministic_commit" && -n "$deterministic_image" && -n "$deterministic_digest" ]] || candidate_matched=0
  [[ -n "$deterministic_github_image" && -n "$deterministic_github_digest" ]] || candidate_matched=0
  [[ -n "$deterministic_azure_image" && -n "$deterministic_azure_digest" ]] || candidate_matched=0
  [[ -n "$deterministic_gitlab_image" && -n "$deterministic_gitlab_digest" ]] || candidate_matched=0
  candidate_reference=""
  for provider in github azure-pipelines gitlab; do
    ...
    if [[ ! -f "$candidate" ]] \
      ...
      || [[ $(jq -r .dedaDigest "$candidate") != "$deterministic_digest" ]] \
      || [[ $(jq -r .githubRunnerImage "$candidate") != "$deterministic_github_image" ]] \
      || [[ $(jq -r .githubRunnerReferenceDigest "$candidate") != "$deterministic_github_digest" ]] \
      || [[ $(jq -r .azureRunnerImage "$candidate") != "$deterministic_azure_image" ]] \
      || [[ $(jq -r .azureRunnerReferenceDigest "$candidate") != "$deterministic_azure_digest" ]] \
      || [[ $(jq -r .gitlabRunnerImage "$candidate") != "$deterministic_gitlab_image" ]] \
      || [[ $(jq -r .gitlabRunnerReferenceDigest "$candidate") != "$deterministic_gitlab_digest" ]]; then
      candidate_matched=0
    fi
...
jq -s --arg deterministic "$deterministic" --arg candidateMatched "$candidate_matched_arg" --argjson candidate "$candidate_json" '
  {providers: ., fullDeterministicQualification:$deterministic, candidateMatched:($candidateMatched == "true"), candidate:$candidate}
  | .providerStatuses = [.providers[] | .status]
  | .releaseQualification =
      (if any(.providerStatuses[]; . == "FAIL") then "FAIL"
       elif .candidateMatched != true then "NOT_QUALIFIED"
       elif .fullDeterministicQualification != "PASS" then "NOT_QUALIFIED"
