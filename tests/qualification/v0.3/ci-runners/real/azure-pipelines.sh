...
for id in "${run_ids[@]}"; do
  ...
  while :; do curl --fail --silent --show-error -u ":$token" "$base/$id?api-version=7.1" | jq '{id,state,result,createdDate,finishedDate}' > "$file"; record_scale "$timeline" "$service_name" azure-pipelines; [[ $(jq -r .state "$file") == completed ]] && break; (( SECONDS < end )) || { write_real azure-pipelines FAIL "Timed out waiting for run $id."; exit 1; }; sleep 5; done
