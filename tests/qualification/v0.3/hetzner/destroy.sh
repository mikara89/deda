...
yes=0
[[ "${1:-}" == --yes ]] && yes=1
[[ $# -eq 0 || ( $# -eq 1 && "$1" == --yes ) ]] || die 'Usage: ./destroy.sh --run <RUN_ID> [--yes]'
...
run=$(safe_label_value "$RUN_ID")
matches() {
  local kind=$1
  hcloud "$kind" list -o json | jq -r --arg run "$run" \
    '.[] | select(.labels.purpose == "deda-qualification" and .labels.version == "v0-3" and .labels.run == $run) | .name'
}

servers=$(matches server)
networks=$(matches network)
firewalls=$(matches firewall)
groups=$(matches placement-group)
keys=$(matches ssh-key)

printf 'RUN_ID: %s\nFilter: purpose=deda-qualification version=v0-3 run=%s\nServers:\n%s\nNetwork:\n%s\nFirewall:\n%s\nPlacement group:\n%s\nSSH key:\n%s\n' \
  "$RUN_ID" "$run" "${servers:-<none>}" "${networks:-<none>}" "${firewalls:-<none>}" "${groups:-<none>}" "${keys:-<none>}"

if [[ "$yes" != 1 ]]; then
  read -r -p "Delete only these exact labelled resources? [y/N] " reply
  [[ "$reply" =~ ^[Yy]$ ]] || { note 'Nothing deleted.'; exit 0; }
fi
...
for runtime_candidate in "$RUNTIME_ROOT" "$LEGACY_RUNTIME_ROOT"; do
  ...
  if [[ -d "$runtime_candidate" ]]; then
    runtime_abs=$(cd "$runtime_candidate" && pwd -P)
    local_state="$runtime_abs/$RUN_ID"
...
note 'Local evidence under tests/qualification/results/ was not deleted.'
